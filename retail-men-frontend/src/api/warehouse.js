 ...
 * @param warehouseData Object containing updated warehouse details
 * @returns { data: { …warehouse fields… } }
 */
export const updateWarehouse = async (warehouseId, warehouseData, token) => {
  const res = await fetch(`${WAREHOUSE_URL}/${warehouseId}`, {
    method: 'PUT',
    headers: {
      'Content-Type': 'application/json',
 ...
 */
export const deleteWarehouse = async (warehouseId, token) => {
  const res = await fetch(`${WAREHOUSE_URL}/${warehouseId}`, {
    method: 'DELETE',
    headers: {
      'Authorization': `Bearer ${token}`
